...
// ---------------------------------------------------------------------
// %COPYRIGHT_BEGIN%
// <copyright file="MLWebRTCExample.cs" company="Magic Leap, Inc">
//
...
namespace MagicLeap.Examples
{
    ...
    public class MLWebRTCExample : MonoBehaviour
    {
        ...
        private MLCamera.MRQuality selectedMRQuality = MLCamera.MRQuality._972x1080;
        ...
        public void Login()
        {
            ...
            try
            {
                webRequestManager.HttpPost(serverURI + "/login", string.Empty, (AsyncOperation asyncOp) =>
                {
        ...
        private void CreateLocalMediaStream()
        {
            ...
            {
                Debug.LogError("Can't use the cameraa for stream because CAMERA permission hasn't been granted");
                return;
            }
            if (!MLPermissions.CheckPermission(MLPermission.RecordAudio).IsOk)
            {
                Debug.LogError("Can't use the microphone for stream because RECORD_AUDIO permission hasn't been granted");
                return;
            }
            ...
            connectContext = MLCamera.ConnectContext.Create();
            connectContext.CamId = MLCamera.Identifier.Main;
            connectContext.Flags = selectedFlag;
            ...
            connectContext.MixedRealityConnectInfo.MRQuality = selectedMRQuality;
            connectContext.MixedRealityConnectInfo.FrameRate = MLCamera.CaptureFrameRate._60FPS;
            ...
            mlCamera = MLCamera.CreateAndConnect(connectContext);
            if(mlCamera == null)
            {
                return;
            }
            
            var streamCapabilities = MLCamera.GetImageStreamCapabilitiesForCamera(mlCamera, MLCamera.CaptureType.Video, MLCamera.CaptureType.Preview);
            ...
            streamConfigs.Add(MLCamera.CaptureStreamConfig.Create(MLCamera.GetBestFitStreamCapabilityFromCollection(streamCapabilities, width, height, MLCamera.CaptureType.Video), MLCamera.OutputFormat.YUV_420_888));
            if(MLCamera.IsCaptureTypeSupported(mlCamera, MLCamera.CaptureType.Preview))
            {
                streamConfigs.Add(MLCamera.CaptureStreamConfig.Create(MLCamera.GetBestFitStreamCapabilityFromCollection(streamCapabilities, width, height, MLCamera.CaptureType.Preview), MLCamera.OutputFormat.YUV_420_888));
            }

            MLCamera.CaptureConfig captureConfig = new MLCamera.CaptureConfig()
            {
                CaptureFrameRate = connectContext.MixedRealityConnectInfo.FrameRate,
            ...
            {
                localVideoSource = MLWebRTC.MLCameraVideoSource.CreateLocal(mlCamera, captureConfig, out MLResult result, id, localVideoRenderer);
                if (MLResult.DidNativeCallSucceed(result.Result, nameof(MLWebRTC.MLCameraVideoSource.CreateLocal)))
                {
            ...
            if (selectedFlag != MLCamera.ConnectFlag.CamOnly)
            {
                // Turn off the local video track behavior when streaming virtual content
        ...
        {
            selectedMRQuality = (MLCamera.MRQuality)(sizeDropdownIndex + 1);
        }
