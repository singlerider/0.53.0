...
// ---------------------------------------------------------------------
// %COPYRIGHT_BEGIN%
// <copyright file="CameraCaptureExample.cs" company="Magic Leap, Inc">
//
...
namespace MagicLeap.Examples
{
    ...
    public class CameraCaptureExample : MonoBehaviour
    {
        ...
        private async void StartVideoCapture()
        {
            ...
            if (RecordToFile)
            {
                var result = await RequestPermissions(MLPermission.RecordAudio, MLPermission.ReadExternalStorage, 
                    MLPermission.WriteExternalStorage);
            ...
            else
            {
                ...
                var result = await RequestPermissions(MLPermission.RecordAudio);
        ...
        private async Task<bool> RequestPermissions(params string[] permissions)
        {
            var requestResults = await Task.WhenAll(permissions.Select(RequestPermission));
            return requestResults.Aggregate(true, (current, requestResult) => current & requestResult);
        }
        ...
        private void SetupQuality()
        {
            ...
            qualityDropDown.AddOptions(MLCamera.MRQuality._648x720, MLCamera.MRQuality._960x720, MLCamera.MRQuality._972x1080, MLCamera.MRQuality._1440x1080);
        ...
        private void SetUpFrameRateDropDown()
        {
            ...
            bool CanDisplay60FPS()
            {
                return
                    ...
                    (ConnectFlag == MLCamera.ConnectFlag.CamOnly || (MRQuality != MLCamera.MRQuality._1944x2160 && MRQuality != MLCamera.MRQuality._2880x2160));
            }
        ...
        private void OnPermissionDenied(string permission)
        {
            ...
            {
                // start video capture regardless, just audio wont be recorded in the file.
                StartPreview();
            }
        }
