...
public class VoiceIntentsExample : MonoBehaviour
{
    ...
    private void Start()
    {
        ...
        if (!MLResult.DidNativeCallSucceed(MLPermissions.RequestPermission(MLPermission.VoiceInput, permissionCallbacks).Result, nameof(MLPermissions.RequestPermission)))
        {
            // can only happen in ZI
            Debug.LogError($"Request for {MLPermission.VoiceInput} failed.");
        }
    }
