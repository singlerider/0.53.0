...
namespace UnityEngine.XR.MagicLeap
{
    ...
    public sealed partial class MLCamera
    {
        ...
        private MLResult.Code InternalConnect(ConnectContext cameraConnectContext)
        {
            ...
            //result = Settings.PopulateCharacteristics(Handle);
        ...
        private MLResult InternalGetIntrinsicCalibrationParameters(ref IntrinsicCalibrationParameters outParameters)
        {
#if UNITY_MAGICLEAP || UNITY_ANDROID
            var parameters =
                NativeBindings.MLCameraIntrinsicCalibrationParameters.Create(outParameters.Width, outParameters.Height);
            var resultCode = NativeBindings.MLCameraGetIntrinsicCalibrationParameters(Handle, ref parameters);

            if (MLResult.DidNativeCallSucceed(resultCode,
                nameof(NativeBindings.MLCameraGetIntrinsicCalibrationParameters)))
            {
                outParameters = new IntrinsicCalibrationParameters()
                {
                    Width = parameters.Width,
                    Height = parameters.Height,
                    FocalLength = parameters.FocalLength.ToVector2(),
                    PrincipalPoint = parameters.PrincipalPoint.ToVector2(),
                    FOV = parameters.Fov,
