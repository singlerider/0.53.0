    ...
    public partial class MLWebRTC
    {
        ...
        public partial class VideoSink
        {
            ...
            public partial struct Frame
            {
                ...
                /// </summary>
                public static readonly Dictionary<OutputFormat, int> NativeImagePlanesLength = new Dictionary<OutputFormat, int>()
                {
                ...
                internal static Frame Create(ulong id, NativeBindings.MLWebRTCFrame nativeFrame, PlaneInfo[] imagePlanes = null)
                {
                    ...
                    {
                        frame.ImagePlanes = (imagePlanes == null) ? new PlaneInfo[nativeFrame.FrameData.ImageFrameInfo.PlaneCount] : imagePlanes;
                        for (ushort i = 0; i < nativeFrame.FrameData.ImageFrameInfo.PlaneCount; ++i)
                        {
                            frame.ImagePlanes[i] = VideoSink.Frame.PlaneInfo.Create(nativeFrame.FrameData.ImageFrameInfo.ImagePlanes[i].Width, nativeFrame.FrameData.ImageFrameInfo.ImagePlanes[i].Height, nativeFrame.FrameData.ImageFrameInfo.ImagePlanes[i].Stride,
                                nativeFrame.FrameData.ImageFrameInfo.ImagePlanes[i].BytesPerPixel, nativeFrame.FrameData.ImageFrameInfo.ImagePlanes[i].Size, nativeFrame.FrameData.ImageFrameInfo.ImagePlanes[i].ImageDataPtr);
                        }
                    }
